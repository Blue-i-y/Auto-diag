...
# Initialisation de la classe Scanner qui hérite des fonctionnalités de Network.

class Scanner(Network):
    def __init__(self):
    
# Lance un scan Nmap sur l'hôte spécifié et enregistre les résultats en format JSON et Excel.
...
# Extrait les services uniques à partir des résultats d'un scan Nmap sauvegardés en JSON.
...
# Récupère les exploits et les shellcodes à partir des données JSON pour un service donné.
...
# Aggrège les exploits et les shellcodes de plusieurs fichiers JSON.
...
# Affiche les résultats d'un scan Nmap de manière formatée pour faciliter la lecture.
...
# Propose des attaques ciblées en fonction du service détecté sur un port donné.
...
# Détecte les services potentiellement vulnérables sur un hôte et propose des actions de sécurité.
...
# Exécute un diagnostic automatique complet, incluant la découverte des hôtes, les scans Nmap, 
#la recherche de vulnérabilités et la génération de rapports.
    def auto_diag(self):
        services = []
        Exploits = []
        Shells = []
